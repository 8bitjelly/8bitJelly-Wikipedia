...
- Input is checked directly inside [`Update()`](/coding-standards/best-practices/avoid/update).
...
### 6. Constant Polling
- Input checks run **every frame** in [`Update()`](/coding-standards/best-practices/avoid/update).
...
- Instead of polling in [`Update()`](/coding-standards/best-practices/avoid/update), you can subscribe to callbacks like `performed`, `canceled`, or `started`.
